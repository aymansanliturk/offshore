export async function onRequest(context) {
  // 1. Absolute top-level catch to prevent Cloudflare 500 HTML pages
  try {
    const { env } = context;
    ...
    if (!API_KEY) {
      return new Response(JSON.stringify({ 
        ...
        details: "GEMINI_API_KEY is missing in your Cloudflare Dashboard. Please add it and REDEPLOY." 
    ...
    for (const model of MODELS) {
      try {
        response = await callGemini("Summarize latest offshore wind news in Denmark in English.", model, true);
        if (response.ok) { successfulModel = model; break; }
      } catch (e) {}
    }

    // PASS 2: Fallback (Knowledge mode)
    if (!successfulModel) {
      isFallback = true;
      for (const model of MODELS) {
        try {
          response = await callGemini("Report on major offshore wind developments in Denmark from your knowledge in English.", model, false);
          if (response.ok) { successfulModel = model; break; }
        } catch (e) {}
      }
    }
    ...
    if (!successfulModel) {
      ...
      return new Response(JSON.stringify({ error: "All Models Failed", details: errorText.slice(0, 300) }), { status: 502, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    // Catch-all for any runtime crashes
    return new Response(JSON.stringify({ error: "Backend Crash", details: e.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
